...
import { Check, ClipboardPaste, Copy, Download, FileCode2, FileJson2 } from "lucide-react";
...
export function ExportDialog({
    ...
    const [tab, setTab] = useState<"sdk" | "instructions" | "plan">("sdk");
    ...
    const label =
        tab === "sdk" ? "SDK TypeScript" : tab === "instructions" ? "Copilot CLI instructions" : "Plan JSON";
    const disabled =
    ...
    function download() {
        ...
        try {
            const name = downloadName(
                plan,
                tab === "sdk" ? "ts" : tab === "instructions" ? "copilot-cli.md" : "plan.json",
            );
    ...
    return (
        <Modal
            ...
            title="Export plan, code & CLI instructions"
            description="Keep the reversible plan, inspect the TypeScript reference, or copy an implementation task into Copilot CLI. Nothing is executed here."
            size="wide"
        >
            <div className="hb-legacy-export-note">
                <Notice title="For a complete language-specific project, use Build & run">
                    These exports are a planner file, TypeScript reference, or existing-repository
                    implementation task—not the selected language&apos;s complete bootstrap. Build &amp; run
                    includes dependencies, entrypoint, host notes, and install/preflight/run steps.
                    {onBuild && (
            ...
            <div className="hb-export-layout">
                <div className="hb-export-main">
                    <Tabs.Root
                        value={tab}
                        onValueChange={(value) => {
                            if (value !== "sdk" && value !== "instructions" && value !== "plan") return;
                            setTab(value);
                            setFeedback(null);
                        }}
                    >
                        <Tabs.List className="hb-tabs-list" aria-label="Export format">
                            ...
                            <Tabs.Trigger className="hb-tab" value="instructions">
                        ...
                        <div className="hb-export-toolbar">
                            <Badge>
                                {tab === "sdk"
                                    ...
                                    : tab === "instructions"
                                      ? "Paste-ready implementation task"
                                      : "Reversible planner format"}
                            </Badge>
                        ...
                        <Tabs.Content value="instructions" className="hb-export-tab-content">
                            <p className="hb-export-explainer">
                                Paste this task into a Copilot CLI session opened at the root of your
                                application. It asks Copilot to inspect and integrate with the existing
                                codebase rather than overwrite it.
                            </p>
                            <textarea
                                className="hb-code-preview"
                                aria-label="Copilot CLI integration instructions"
                                readOnly
                                spellCheck={false}
                                wrap="off"
