...
function ServerRow({ server, index, edit, issues }: EditorProps & { server: McpServer; index: number }) {
    ...
    return (
        <details
            className="hb-item-card hb-mcp-server-card"
            aria-label={`MCP server ${server.name || index + 1}`}
        >
            <summary>
                <span className="hb-item-title">
                    <Network size={16} aria-hidden="true" />
                    <span>
                        <strong>{server.name || `Remote service ${index + 1}`}</strong>
                        <small>
                            {server.tools.length} reviewed {server.tools.length === 1 ? "tool" : "tools"}
                        </small>
                    </span>
                </span>
                <Badge>HTTP</Badge>
            </summary>
            <div className="hb-mcp-server-body">
                <div className="hb-item-heading">
                    <h4>Connection and allowlist</h4>
                    <Button
                        variant="ghost"
                        size="small"
