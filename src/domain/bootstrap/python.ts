        ...
        return {
            ...
            notes: [
                ...
                plan.policy.permissionMode === "host"
                    ? "Extend host.py: register PERMISSION_HANDLER and tool handlers in TOOL_HANDLERS, then assign selected PRE_TOOL_HOOK/POST_TOOL_HOOK/SESSION_FS_FACTORY integrations. Preflight rejects every missing selected integration; no success-shaped stub is supplied."
                    : "This project explicitly binds PermissionHandler.approve_all. It approves ordinary permission requests once; managed policy, content exclusion, downstream authorization, invalid tools, and sandbox enablement remain authoritative, while enabled sandbox bypass can also be approved. Other selected host integrations still fail preflight until implemented.",
                plan.model.provider === "copilot" && plan.identity === "s2s-installation"
                    ? plan.target.runtime === "external"
